   ...
   module.exports = {
      ...
      _openSync: function(p, flags, mode) {
         ...
         if(it) {
            ...
            if((flags == O_RDONLY || flags & O_RDWR) && !actrl.canRead(it, this)) {
               throw new Error('EACCESS');
            }

            if((flags & O_RDWR || flags & O_WRONLY) && !actrl.canWrite(it, this)) {
               throw new Error('EACCESS');
            }

            if(flags & O_TRUNC) {
               if(!actrl.canWrite(it, this)) {
                  throw new Error('EACCESS');
               }
      ...
      _writeSync: function(fd, buffer, offset, length, position) {
         ...
         fd.item.mtime = fd.item.ctime = +new Date();
