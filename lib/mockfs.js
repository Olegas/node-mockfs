var fs = require('fs'),
    ...
    actrl = require('./access-control.js'),
...
(function(){
   ...
   function MockFS(spec) {
      ...
      this._mode = spec.mode || parseInt('0777', 8);
      this._uid = spec.uid || 0;
      this._gid = spec.gid || 0;
      this._options = {};
   ...
   MockFS.prototype._removeFSItem = function(p) {
      ...
      if(actrl.canWrite(parent, this)) {
         parent.mtime = parent.ctime = +new Date();
      ...
      while(p.length) {
         ...
         if(pathItem) {
            if(root && root.items && pathItem in root.items) {
               ...
               if(this._isDirectory(root) && !actrl.canSearch(root, this)) {
                  throw new Error('EACCESS');
               }
            } else {
   ...
   fs.readdirSync(path.join(__dirname, '/plugins')).forEach(function(f){
      if(f == 'readfile.js' || f == 'writefile.js' || f == 'appendfile.js')
         return;
